...
static void log_printf(const char *pMsg, ...)
{
	...
	{
		FILE *pFile;
		fopen_s(&pFile, s_log_filename, "rb");
		if (pFile)
		{
			fprintf(pFile, "%s", buf);
			fclose(pFile);
...
int main(int arg_c, char* ppArgs[])
{
	...
	timeFile.open("OpenMP_Static_50_1.csv");
	...
	for (int i = 0; i < 200; ++i)
	{
		...
		if (test_memory_compression)
		{
			...
			FILE *pFile;
			fopen_s(&pFile,pDst_filename, "wb");
			if (!pFile)
			{
		...
		//output total time for compression and uncompression
		timeFile << total_comp_time + total_uncomp_time << std::endl;
